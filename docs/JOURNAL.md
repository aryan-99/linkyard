...
- **Phase:** v1.0 — preparing tag. Repo is public. Extension distributed via load-unpacked (no CWS). Fly.io live-demo scaffolding shipped.
...
- **Demo:** `DEMO_SEED=true` env var (default false) auto-seeds 39 pre-fetched diverse links on first boot via `backend/app/seed/`. Idempotent (no-op once links exist). Wired into `docker-compose.prod.yml` (`${DEMO_SEED:-false}`) and `fly/backend/fly.toml` (=`true`). Fly deploy paths: `fly/backend/fly.toml`, `fly/frontend/fly.toml`, `fly/frontend/Dockerfile`. Full walkthrough in `docs/DEPLOY.md`. Egress firewall now gated by `EGRESS_FIREWALL` env var (default `true` for prod-compose, `false` on Fly because Machines deny CAP_NET_ADMIN).
