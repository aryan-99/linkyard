...
- **Phase:** v1.0 — ready to tag. Repo is public. Extension distributed via load-unpacked (no CWS). **Hosted demo dropped from v1 scope** (2026-06-22): `docs/DEPLOY.md` deleted and README "Live demo / on the way" section removed — shipping what exists rather than carrying an unkept promise. The auto-seed pipeline is **kept and reframed** as a "try it with sample data" feature (inert by default, decoupled from any hosting choice). A hosted demo can return later on the user's rented VM without blocking v1.
...
- **Sample data:** `DEMO_SEED=true` env var (default false) auto-seeds 39 pre-fetched diverse links on first boot via `backend/app/seed/`. Idempotent (no-op once links exist). Wired into `docker-compose.prod.yml` (`${DEMO_SEED:-false}`). Reframed in README as a "try it with sample data" feature, not a hosted demo. Egress firewall gated by `EGRESS_FIREWALL` env var (default `true` for prod-compose; set `false` on any managed host that denies CAP_NET_ADMIN — app-level SSRF guards in `services/fetch.py` remain primary defence).
...
- **Docs:** `HOW_IT_WORKS.md` (conceptual design). README has Screenshots row + "Try it with sample data" section.
...
- **Next logical slice:** Tag v1.0 — `git tag v1.0.0 && git push --tags`. v1.1: OpenAI provider unlock (1536-dim migration + API key encryption). Hosted demo deferred indefinitely (would live on the user's rented VM; not in v1 scope) — if revived, candidates were Neon (Postgres+pgvector) + Render web service + static frontend host, and CORS must be pinned to `"https://<frontend-host>|chrome-extension://[a-p]{32}"`. Footer portfolio link deferred until portfolio site is built.
...
### 2026-06-22 — Dropped hosted demo from v1 scope; reframed seed as sample data (architect)
Decided to ship existing as v1 rather than wait on a hosted demo. Deleted `docs/DEPLOY.md` (the platform-undecided TODO stub) and removed the README "Live demo / on the way" section — an unkept promise is worse than none. **Kept** the auto-seed pipeline (`backend/app/seed/`, `DEMO_SEED` flag, lifespan hook, compose wiring, 4 tests) untouched: it's inert by default, tested, and decoupled from any hosting choice. Reframed it in README as "Try it with sample data" (set `DEMO_SEED=true` before `make up`). Hosted demo can return later on the user's rented VM without blocking v1. No code changed — docs only.

### 2026-06-20 — Removed Fly.io; deploy platform undecided (architect)
Fly.io discontinued its permanent free tier for new accounts (now pay-as-you-go; a 3-piece backend+frontend+Postgres demo runs ~$5–15/mo — not worth it for a portfolio demo; user will rent a shared VM for multiple projects instead, separate from this). Deleted `fly/` (`fly/backend/fly.toml`, `fly/frontend/fly.toml`, `fly/frontend/Dockerfile`). Rewrote `docs/DEPLOY.md` as a platform-agnostic TODO stub carrying **verified June-2026 free candidates**: Neon (Postgres+pgvector, permanent free, no card, scale-to-zero 5min), Render web service (backend; free, no card, 750 hrs/mo, ~1min cold start — but its own free Postgres is deleted after 30 days, so DB must go to Neon), static host (Render/Vercel/Netlify/Cloudflare Pages). Genericized Fly-specific framing in `backend/Dockerfile` (kept the `EGRESS_FIREWALL` toggle — still needed on any CAP_NET_ADMIN-less host) and `.env.example`. README Live-demo section reworded ("on the way"). Build artifacts unchanged (`backend/Dockerfile`, frontend `npm run build`); `docker-compose.prod.yml` self-host path untouched.

### 2026-06-13 — v1.0 demo: auto-seed + Fly.io deploy scaffolding (backend + devops + architect)
**Backend:** Added automated demo seed at `backend/app/seed/` — `demo_links.json` (39 pre-fetched entries: Python, Go, Rust, Haskell, Clojure, Elixir, ML/AI, databases, design, philosophy, science, productivity, security, algorithms) + idempotent `seed_demo_links()` (COUNT guard, parallel `asyncio.gather` embedding, `source="seed"`) + gated startup lifespan hook in `main.py` keyed off new `DEMO_SEED` setting (default False). Generator at `backend/scripts/generate_seed.py` for future re-runs. 4 new unit tests. 3 URLs dropped during generation (MIT Press SICP 403, NNGroup 404, Neovim doc empty parse).
**Devops:** `fly/backend/fly.toml` + `fly/frontend/fly.toml` (auto-stop Machines, shared-cpu-1x, healthchecks). `fly/frontend/Dockerfile` (multi-stage node:20-alpine build + nginx:1.27-alpine serve, `VITE_API_BASE_URL` baked as build arg). `docs/DEPLOY.md` — 12-step walkthrough from `fly auth login` through Postgres attach, `CREATE EXTENSION vector`, alembic upgrade via ssh, frontend deploy, CORS pin, DEMO_SEED disable.
**Architect:** Fixed pre-existing `backend/Dockerfile` build-context bugs: COPY paths were `requirements.txt`, `app/`, `../scripts/...` (broken regardless of context choice) — repaired to repo-root-relative (`backend/requirements.txt`, `backend/app/`, `scripts/...`). Also added `COPY backend/alembic.ini` + `COPY backend/alembic/` so `alembic upgrade head` works inside the container (DEPLOY.md step 8 depends on this). Gated egress script behind `EGRESS_FIREWALL` env var (default `true`; Fly sets `false` because Machines deny CAP_NET_ADMIN — app-level SSRF guards in `services/fetch.py` remain primary defence). Added `DEMO_SEED` to `.env.example` + `docker-compose.prod.yml`. README: Screenshots row (links/search/settings + extension) + Live demo section pointing at DEPLOY.md. Gotcha: Fly's managed Postgres needs `ghcr.io/baosystems/postgis:16-3.4` for pgvector; the official `pgvector/pgvector` image requires a manual Machine deploy. CORS_ORIGIN_REGEX in `fly/backend/fly.toml` remains `__PLACEHOLDER__` until user has the frontend `.fly.dev` URL post-deploy.

### 2026-05-18 — Extension v1.0.0 icons + manifest wiring (architect)
Icon (AI-generated, temp) converted from JPG to PNG at 16/48/128px for extension toolbar and 512px + favicon.ico for the frontend. Wired into `manifest.json` (`icons` + `action.default_icon`) and `index.html`. Original kept in `assets/`. Extension version bumped to `1.0.0`. Extension distributed via load-unpacked — no CWS listing.

### 2026-05-17 — Frontend polish v1.0 (frontend + architect)
Added persistent `Footer` component: "Built by Aryan Mittal" (plain text; portfolio link deferred) + GitHub SVG icon linking to `https://github.com/aryan-99/linkyard`. `App.tsx` uses flex-column layout so footer sticks to bottom. `SettingsPage`: permanent coming-soon notice for OpenAI provider; amber warning banner (`--color-warning` / `--color-warning-tint`) appears when dropdown differs from saved provider value. Primary buttons (`AddLinkForm`, Settings save, ConfirmDialog confirm) switched from solid blue to outlined accent style (transparent bg, `1.5px solid --color-accent`, accent text) — fits dark mode palette better. Two new CSS variables added to both themes in `index.css`. Build clean, `tsc` clean.
...
Added `docker-compose.prod.yml`: db (pgvector/pgvector:pg16) + backend built from `backend/Dockerfile`; `cap_add: NET_ADMIN` for iptables egress script; backend waits on db healthcheck; named volume `linkyard-prod-db-data`. Added `.env.example` documenting all runtime env vars. Added `Makefile`: `build / up / down / migrate / logs / help` (default). `CORS_ORIGIN_REGEX` defaults to dev wildcard — must be pinned to load-unpacked extension ID before go-live. `ADMIN_TOKEN` placeholder is `changeme` — must be replaced before go-live.
...
**Devops note:** Set `CORS_ORIGIN_REGEX=chrome-extension://<extension-id>` in prod environment before go-live. Find the ID on `chrome://extensions` after loading unpacked.
